...
# An on_rollback function is defined to delet the file if the data quality check fails.
@save_raw_data_to_file.on_rollback
def del_file(txn):
    '''Deletes file.'''
    print("YAY I was TRIGGERED!")
    os.unlink(txn.get("filepath"))

# A quality_test taks is definded to  check if ther is sufficient game data in the file.
@task
...
def mlb_flow_rollback(team_name, start_date, end_date):
    ...
    # Create transaction which will be used to rollback if the data quality is te..
...
if __name__ == "__main__":
    ...
    # Uncomment this line to kick of a flow run that will fail the data quality check
